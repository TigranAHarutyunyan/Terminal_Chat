...
class Server {
    public:
        ...
        static std::string xorEncryptDecrypt(const std::string& text, char key);
    private:
        ...
        void read_handler(const boost::system::error_code &err, std::shared_ptr<boost::asio::ip::tcp::socket> socket, std::shared_ptr<boost::asio::streambuf> buf, size_t bytes_transferred);
        void do_accept(std::shared_ptr<boost::asio::ip::tcp::socket> socket, boost::asio::ip::tcp::acceptor &acceptor, boost::asio::io_context &ioContext);
